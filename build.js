...
if (!html.includes(oldENV)) {
  const idx = html.indexOf('const ENV = {');
  console.error('Could not find ENV block in index.html');
  if (idx !== -1) {
    console.error('Found ENV at offset', idx, '— nearby content:');
    console.error(html.slice(idx, idx + 300));
  }
  process.exit(1);
}
...
console.log('NAS_URL: ', NAS_URL  ? 'set: ' + NAS_URL  : '** MISSING — proxy will 500 **');
